...
def make_pipeline():
    ...
    match_country = { "$match" : { "country" : { "$eq" : 'India' } } }
    match_lon = { "$match" : { "lon" : { "$gte" : 75, "$lte" : 80} } }
    match_combine = { "$match" : { "country" : { "$eq" : 'India' }, "lon" : { "$gte" : 75, "$lte" : 80 } } }
    group_region = { "$group" : { "_id" : "$isPartOf", "count" : { "$sum" : 1 } } }
    sort_desc = { "$sort" : { "count" : -1 } }

    pipeline = [unwind_isPartOf, match_combine, group_region, sort_desc]
    return pipeline
