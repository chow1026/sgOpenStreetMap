...
INPUT_OSM_FILE = "singapore_sg_9.osm"  # Replace this with your osm file
OUTPUT_OSM_FILE = "singapore_fixed_data_9.osm"
...
KEY_SWAPS['addr:city'] = {'#01-05': 'addr:unit', '#01-06': 'addr:unit', '#01-33': 'addr:unit', '#01-38/40/42': {'addr:unit': '#01-38/#01-40,#01-42'}, '#01-44': 'addr:unit', '#01-46': 'addr:unit', '#01-50': 'addr:unit', '#01-58/60': {'addr:unit': '#01-58/#01-60'}, '#01-62': 'addr:unit', 'Ang Mo Kio': 'addr:suburb', 'Changi Village': 'addr:suburb', 'Holland Village': 'addr:suburb', 'Punggol': 'addr:suburb', 'Sembawang': 'addr:suburb', 'Woodlands Spectrum II': 'addr:housename', '01-169 Singapore': {'addr:unit': '#01-169', 'addr:city': 'Singapore'} }
...
def replace_key(elem):
    for tag in elem.iter("tag"):
        kval = tag.attrib['k']
        vval = tag.attrib['v']
        for kys, value_newkys in KEY_SWAPS.items():
            if vval in value_newkys.keys() and kval == kys:
                # print(tag.attrib['k'])
                # print(tag.attrib['v'])

                print('k: {0}, v: {1}, kys: {2}, newky: {3}'.format(kval, vval, kys, value_newkys[vval]))
                # elem.remove(tag)
                if isinstance(value_newkys[vval], dict):
                    for newk, newv in value_newkys[vval].items():
                    ...
                    if value_newkys[vval] == 'rm':
                        elem.remove(tag)
                    else:
                        tag.set('k', value_newkys[vval])
