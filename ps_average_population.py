...
def make_pipeline():
    ...
    # pipeline = [
    #     { "$match" : {  "country" : { "$ne" : None }, "population" : { "$ne" : None }, "isPartOf" : { "$ne" : None } } },
    ...
    # ]
    match_onlyExists = { "$match" : {  "country" : { "$exists" : 1 }, "population" : { "$exists" : 1 }, "isPartOf" : { "$exists" : 1 } } }
    unwind_isPartOf = { "$unwind" : "$isPartOf" }
    group1_dual_id = { "$group" : {
            "_id" : { "region" : "$ispartOf" , "country" : "$country" },
            "avg_population" : { "$avg" : "$population" },
            "countries" : { "$addToSet" : "$country" } } }
    group2_dual_id = { "$group" : {
            "_id" : "$id.country",
            "avgRegionalPopulation" : { "$avg" : "$avg_population" }
            } }
    group1_w_country = { "$group" : {
            "_id" : "$isPartOf",
            "countries" : { "$addToSet" : "$country" },
            "regions" : { "$addToSet" : "$isPartOf" },
            "city_avg" : { "$avg" : "$population"}
            } }
    group1_wo_country = { "$group" : {
            "_id" : "$isPartOf",
            "regions" : { "$addToSet" : "$isPartOf" },
            "city_avg" : { "$avg" : "$population"}
            } }
    unwind_region = { "$unwind" : "$regions" }
    group2_w_country = { "$group" : { "_id" : "$regions", "countries" : { "$addToSet" : "$country" }, "region_avg" : { "$avg" : "$city_avg" } } }
    group2_wo_country = { "$group" : { "_id" : "$countries", "avgRegionalPopulation" : { "$avg" : "$city_avg" } } }
    unwind_country = { "$unwind" : "$countries" }
    group3 = { "$group" : { "_id" : "$countries", "country_avg" : { "$avg" : "$region_avg" } } }

    # This is the  incorrect but demonstrated the dual-id usage
    pipeline = [match_onlyExists, unwind_isPartOf, group1_dual_id, unwind_country]
    # This is the correct one
    pipeline = [match_onlyExists, unwind_isPartOf, group1_w_country, unwind_country, group2_wo_country]
    return pipeline
